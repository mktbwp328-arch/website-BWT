...
(function () {
  ...
  async function save() {
    ...
    // ถ้าติดต่อฐานข้อมูลไม่ได้ตอนเปิดหน้า แปลว่าข้อมูลที่กำลังแก้อาจเก่ากว่าของจริง
    if (window.BWT && window.BWT.dbOk === false &&
        !confirm("เปิดหน้านี้ตอนที่ต่อฐานข้อมูลไม่ได้ ข้อมูลที่เห็นอาจไม่ใช่ชุดล่าสุด\n\n" +
                 "ถ้าบันทึกตอนนี้ อาจเขียนทับงานที่คนอื่นแก้ไว้\nแนะนำให้รีเฟรชหน้าก่อน — ยืนยันจะบันทึกเลยไหม?")) {
      status("ยกเลิกการบันทึก — ลองรีเฟรชหน้าแล้วแก้ใหม่", false);
      return;
    }
  ...
  async function start() {
    ...
    /* ห้ามแก้ตอนที่หน้าเว็บแสดงภาษาอังกฤษ
       เพราะข้อความบนจอเป็นคำแปล ไม่ใช่ข้อมูลจริง ถ้าแก้แล้วบันทึก
       ข้อความอังกฤษจะไปทับเนื้อหาภาษาไทยที่เป็นต้นฉบับ */
    if (window.BWT && window.BWT.lang && window.BWT.lang() === "en") {
      if (confirm("โหมดแก้ไขใช้กับภาษาไทยเท่านั้น\n\n" +
                  "ตอนนี้หน้าเว็บแสดงภาษาอังกฤษ ซึ่งเป็นคำแปล ไม่ใช่เนื้อหาต้นฉบับ\n" +
                  "ถ้าแก้ตอนนี้ ข้อความอังกฤษจะไปทับเนื้อหาภาษาไทย\n\n" +
                  "สลับกลับเป็นภาษาไทยแล้วแก้ต่อเลยไหม?")) {
        window.BWT.setLang("th");   // สลับภาษาแล้วโหลดหน้าใหม่ โหมดแก้ไขจะติดมาเอง
      } else {
        sessionStorage.removeItem(LS_MODE);
        const u = new URL(location.href); u.searchParams.delete("edit");
        location.replace(u.toString());
    ...
    /* รอให้หน้าเว็บดึงเนื้อหาล่าสุดจากฐานข้อมูลให้เสร็จก่อน แล้วค่อยแก้
       แล้วใช้ "ข้อมูลก้อนเดียวกัน" กับที่หน้าเว็บวาดอยู่ (window.BWT.data)
       ไม่ใช่สำเนาของตัวเอง — ไม่งั้นกดบันทึกจะเขียนทับงานที่แก้จากหลังบ้านหรือเครื่องอื่น */
    if (window.BWT && window.BWT.ready) {
      try { await window.BWT.ready; } catch (e) { }
    }
    if (window.BWT && window.BWT.data) D = window.BWT.data;
    toolbar();
    wire();
